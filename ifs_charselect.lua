...
function ifs_charselect_fnBuildScreen(this, mode)
	...
	if ( mode == 0 ) then
		boxw = w * 0.38
		boxh = h * 0.6
		jvfontsize = gPopupButtonFont
		yoffset1 = 16
		yoffset2 = 47
	else
		boxw = w * 0.4
		boxh = h * 0.4
		jvfontsize = gPopupTinyTextFont
		yoffset1 = 32
		yoffset2 = 12
	end
...
--		this.Info.Window.titleBarElement.x = this.Info.Window.titleBarElement.x - 16
		...
		if((gLangStr ~= "english") and (gLangStr ~= "uk_english")) then
			this.Info.Window.titleBarElement.font = gPopupTinyTextFont
			jvfontsize  = gPopupTinyTextFont
		else
			this.Info.Window.titleBarElement.font = gPopupButtonFont
		end
...
--	}
	...
	if(this.Helptext_Accept) then 
		if ( mode == 0 ) then
			...
			this.Helptext_Accept.helpstr.font = gPopupButtonFont
		elseif (ScriptCB_IsHorizontalSplitScreen()) then
			this.Helptext_Back.helpstr.font = gPopupTinyTextFont
			this.Helptext_Accept.helpstr.font = gPopupTinyTextFont
